...
import com.v5ent.game.WorldController;
...
public class Trace {
	...
	public static void symbol(WorldController worldController, List<Vector2> points, Player player){
        ...
        ****************************************************************/
        ...
        if(points.size()>=3){
        	...
        	double p_dir = dirtorad(player.dir);
        	//****************************************************************/
        	if(dir == 3){ //Thunder
        		threshold = 60;//degree
        		...
        		if(size >=2 
        			...
        			Gdx.app.debug(TAG, "At direction:"+(p_dir)+" x:"+(player.getX())+" y:"+(player.getY())+" cos(direction)*200:"+(Math.cos(p_dir)*200)+" sin(direction)*200:"+Math.sin(p_dir)*200);
        			...
        			worldController.skill.setPosition(
        					player.getX() +(float) (Math.cos(p_dir)*200), 
        					player.getY()+ (float)(Math.sin(p_dir)*200));
        			Gdx.app.debug(TAG, " x:"+((float) (Math.cos(p_dir)*200))+" y:"+((float)(Math.sin(p_dir)*200))+" dir:"+radtodeg(p_dir));
        			Gdx.app.debug(TAG, "Skill x:"+(player.getX() +(float) (Math.cos(p_dir)*200))+" y:"+(player.getY()+ (float)(Math.sin(p_dir)*200))+" dir:"+radtodeg(p_dir));
//                om = instance_create(player.x + cos(p_dir)*200, player.y - sin(p_dir)*200,player_magic);//water
        	...
        	if(dir == 1){ //Earth
        		...
        		if(size >1 
        			...
        			worldController.skill.setPosition(
        					player.getX() +(float) (Math.cos(p_dir)*200), 
        					player.getY()+ (float)(Math.sin(p_dir)*200));
//                om = instance_create(player.getX() + Math.cos(p_dir)*200, player.getY() - Math.sin(p_dir)*200,player_magic);//water
...
//               om.damage_value = 95;
	 ...
	 * @return
	 */
	private static double dirtorad(Direction dir) {
		...
		if(Direction.UP_LEFT.equals(dir)){
			return Math.PI*3/4;
		}
		if(Direction.UP_RIGHT.equals(dir)){
			return Math.PI*1/4;
		}
		if(Direction.DOWN_LEFT.equals(dir)){
			return Math.PI*5/4;
		}
		if(Direction.DOWN_RIGHT.equals(dir)){
			return Math.PI*7/4;
		}
		return 0;
	}
