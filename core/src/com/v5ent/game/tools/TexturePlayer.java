 ...
 * File Name:TexturePlayer.java
 * Package Name:com.v5ent.game.tools
 * Date:2014-7-26下午11:23:53
 ...
 * @author Nathan Sweet
 */
public class TexturePlayer implements Disposable {
	...
	public static class TextureAtlasData {
		public static class Page {
			...
			public Page(FileHandle handle, float width, float height,
					boolean useMipMaps, Format format, TextureFilter minFilter,
					TextureFilter magFilter, TextureWrap uWrap,
					TextureWrap vWrap) {
				this.width = width;
				this.height = height;
			...
			try {
				...
				while (true) {
					...
					if (line.trim().length() == 0)
						...
						if (readTuple(reader) == 2) { // size is only optional
														// for an atlas packed
														// with an old
														// TexturePacker.
						...
						pageImage = new Page(file, width, height,
								min.isMipMap(), format, min, max, repeatX,
								repeatY);
						pages.add(pageImage);
					} else {
						...
						if (readTuple(reader) == 4) { // split is optional
							...
							if (readTuple(reader) == 4) { // pad is optional,
															// but only present
															// with splits
								region.pads = new int[] {
										Integer.parseInt(tuple[0]),
				...
				throw new GdxRuntimeException("Error reading pack file: "
						+ packFile, ex);
			} finally {
				StreamUtils.closeQuietly(reader);
			}

			new Sort().sort((Object[]) regions.items,
					(Comparator) indexComparator, 0, regions.size);
		}
	 ...
	 */
	public TexturePlayer(String internalPackFile) {
	 ...
	 */
	public TexturePlayer(FileHandle packFile) {
	 ...
	 */
	public TexturePlayer(FileHandle packFile, boolean flip) {
		this(packFile, packFile.parent(), flip);
	}

	public TexturePlayer(FileHandle packFile, FileHandle imagesDir) {
	 ...
	 *            perspective where 0,0 is the upper left corner.
	 */
	public TexturePlayer(FileHandle packFile, FileHandle imagesDir, boolean flip) {
	 ...
	 *            May be null.
	 */
	public TexturePlayer(TextureAtlasData data) {
		if (data != null)
			load(data);
	}
	...
	private void load(TextureAtlasData data) {
		ObjectMap<com.v5ent.game.tools.TexturePlayer.TextureAtlasData.Page, Texture> pageToTexture = new ObjectMap<com.v5ent.game.tools.TexturePlayer.TextureAtlasData.Page, Texture>();
		for (com.v5ent.game.tools.TexturePlayer.TextureAtlasData.Page page : data.pages) {
		...
		for (com.v5ent.game.tools.TexturePlayer.TextureAtlasData.Region region : data.regions) {
			...
			if (region.name.equals(name)) {
				...
				if (splits == null)
					throw new IllegalArgumentException(
							"Region does not have ninepatch splits: " + name);
				NinePatch patch = new NinePatch(region, splits[0], splits[1],
						splits[2], splits[3]);
				if (region.pads != null)
					patch.setPadding(region.pads[0], region.pads[1],
	...
	static public class AtlasSprite extends Sprite {
		...
		public void setBounds(float x, float y, float width, float height) {
			...
			int packedWidth = region.rotate ? region.packedHeight
					: region.packedWidth;
			int packedHeight = region.rotate ? region.packedWidth
					: region.packedHeight;
			super.setBounds(x + region.offsetX, y + region.offsetY, packedWidth
					* widthRatio, packedHeight * heightRatio);
		}
		...
		public void rotate90(boolean clockwise) {
			...
			if (clockwise) {
				...
				region.offsetY = region.originalHeight * heightRatio
						- oldOffsetX - region.packedWidth * widthRatio;
			} else {
				region.offsetX = region.originalWidth * widthRatio - oldOffsetY
